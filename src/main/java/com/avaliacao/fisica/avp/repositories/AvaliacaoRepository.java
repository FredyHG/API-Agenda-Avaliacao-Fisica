...
public interface AvaliacaoRepository extends JpaRepository<AvaliacaoModel, Long> {
    ...
    @Query(value = "SELECT p FROM avaliacao p WHERE p.clientID = :id AND p.status = false")
    Optional<AvaliacaoModel> findByCpf(Long id);

    @Query(value = "SELECT p FROM avaliacao p WHERE p.clientID = :id AND p.status = true")
